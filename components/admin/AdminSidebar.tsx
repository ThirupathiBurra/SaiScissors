...
import {
  ...
  Image as ImageIcon, Settings, LogOut, Tag, LayoutTemplate,
  PenTool, Menu, X, ChevronLeft, ChevronRight, TrendingUp,
} from 'lucide-react';
import { ROUTES } from '@/constants/routes';
import { useAuth } from '@/hooks/useAuth';
import { useSidebarState } from '@/hooks/useSidebarState';

// Sidebar collapsed context so layout can read it
...
export default function AdminSidebar() {
  ...
  const links = [
    ...
    { name: 'Bookings',  href: ROUTES.ADMIN.BOOKINGS,  icon: CalendarDays },
    { name: 'Services',  href: ROUTES.ADMIN.SERVICES,  icon: Scissors },
    { name: 'Barbers',   href: ROUTES.ADMIN.BARBERS,   icon: Users },
    { name: 'Reviews',   href: ROUTES.ADMIN.REVIEWS,   icon: Star },
    { name: 'Gallery',   href: ROUTES.ADMIN.GALLERY,   icon: ImageIcon },
    { name: 'Offers',    href: ROUTES.ADMIN.OFFERS,    icon: Tag },
    { name: 'Banners',   href: ROUTES.ADMIN.BANNERS,   icon: LayoutTemplate },
    { name: 'Content',   href: ROUTES.ADMIN.CONTENT,   icon: PenTool },
    ...
    <>
      ...
      <div className="p-2 border-t border-[#222]">
        <button
          ...
          className={`flex items-center gap-3 rounded-lg text-sm font-medium text-[#9CA3AF] hover:text-red-400 hover:bg-red-500/10 transition-all w-full ${
            showLabels ? 'px-3 py-2.5' : 'px-2.5 py-2.5 justify-center'
          }`}
        >
  ...
  // Don't render collapsed state until client-side to avoid hydration mismatch
    ...
    <>
        ...
        <div className={`flex items-center border-b border-[#222] ${collapsed && mounted ? 'px-3 py-4 justify-center' : 'px-5 py-4 justify-between'}`}>
          {(!collapsed || !mounted) && (
            <h1 className="text-xl font-heading font-bold text-white whitespace-nowrap overflow-hidden">
              Saiscissors <span className="text-[#C9A84C]">Admin</span>
            </h1>
          )}
          <button
            onClick={toggle}
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-[#111] border-b border-[#222] flex items-center justify-between px-4 h-14">
        <h1 className="text-base font-heading font-bold text-white">
          <span className="text-[#C9A84C]">Admin</span> Panel
        </h1>
        <button
